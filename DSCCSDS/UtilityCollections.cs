...
{ // Updated 2017-09-02
    ...
    public class BimapOneToOne<T1, T2> : IEnumerable<(T1 one, T2 two)>
    {
        ...
        public T1 this[T2 key]
        {
            ...
            set
            {
                ...
                if (d1.TryGetValue(value, out T2 result2))
                { // If 'value' currently has any association...
                ...
                if (d2.TryGetValue(key, out T1 result1))
                { // Same check, for the other half.
                    d1.Remove(result1);
                }
                d1[value] = key;
                d2[key] = value;
    ...
    public class BimapOneToMany<T1, TMany> : IEnumerable<(T1 one, TMany two)>
    {
        ...
        public T1 this[TMany key]
        {
            ...
            set
            {
                ...
                if (d2.TryGetValue(key, out T1 result1))
                { // If 'obj' was already linked to a T1, remove its link with 'obj'.
    ...
    public class OrderingCollection<T> : IEnumerable<T>
    {
        ...
        public ulong Relabelings = 0UL;
        public ulong Insertions = 0UL;
